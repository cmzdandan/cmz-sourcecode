...
import com.cmz.orm.framework.EntityOperation.PropertyMapping;
import com.cmz.orm.util.BeanUtils;
import com.cmz.orm.util.DataUtils;
import com.cmz.orm.util.GenericsUtils;
import com.cmz.orm.util.StringUtils;

/**
 * @author chen.mz
 * @email 1034667543@qq.com
 * @create 2019年7月15日 下午6:04:51
 * @description baseDao增强
 */
public abstract class BaseDaoSupport<T extends Serializable, PK extends Serializable> implements BaseDao<T, PK> {
	...
	protected BaseDaoSupport() {
		try {
			Class<T> entityClass = (Class<T>) GenericsUtils.getSuperClassGenricType(getClass(), 0);
	 ...
	 * 
	 * <pre>
	 * <code> User entity = service.get(1); entity.setName(&quot;zzz&quot;); // 更新对象
	...
	* </code>
	 ...
	 * 
	 * <pre>
	 * <code> Map<String,Object> map = new HashMap(); map.put("name"," 张三 ");
	 * map.put("id","0002"); String sql = "UPDATE SET name = :name WHERE id = :id";
	 * // 更新对象
	 * 
	...
	* </code>
	...
	public Page<T> select(QueryRule queryRule, final int pageNo, final int pageSize) throws Exception {
		...
		String orderSql = bulider.getOrderSql();
		orderSql = (StringUtils.isEmpty(orderSql) ? " " : (" order by " + orderSql));
		String sql = "select " + op.allColumn + " from " + getTableName() + whereSql + orderSql + " limit " + start
				+ "," + pageSize;
	 ...
	 * @return
	 */
	protected Page<Map<String, Object>> selectBySqlToPage(String sql, Map<String, ?> param, final int pageNo,
		...
		List<Map<String, Object>> list = (List<Map<String, Object>>) this.jdbcTemplateReadOnly().queryForList(sql,
				param);
		log.debug(sql);
	...
	* </code>
	 ...
	 *            ID 字段名称
	 * @param isCopyNull
	 *            是否拷贝 null( 当 POJO 中的值为 null 时，如果 isCopyNull=ture, 则用 null, 否则继续使 用
	 *            PO 中的值）
	 */
	@SuppressWarnings("unchecked")
			...
			for (int i = 0; i < fields.length; i++) {
				...
				for (int j = 1; j <= colCount; j++) {
					...
					try {
						...
						log.warn("BeanUtils.copyProperty error, field name: " + f.getName() + ", error: " + e);
					}
				}
			}
		} catch (Exception e) {
	 ...
	 * @return
	 */
	private <T> Page simplePageQuery(String sql, RowMapper<T> rowMapper, Map<String, ?> args, long pageNo,
			long pageSize) {
		long start = (pageNo - 1) * pageSize;
	 ...
	 * @return
	 */
	private <T> Page simplePageQueryByStart(String sql, RowMapper<T> rowMapper, Map<String, ?> args, long start,
			long pageSize) {
		// 首先查询总数
	...
	protected <T> List<T> simpleListQueryByStart(String sql, RowMapper<T> rowMapper, Map<String, ?> args, long start,
			long pageSize) {
		sql = sql + " limit " + start + "," + pageSize;
		log.debug(StringUtils.format("[Execute SQL]sql:{0},params:{1}", sql, args));
		List<T> list = this.jdbcTemplateReadOnly().query(sql, rowMapper, args);
